...
CREATE TABLE `User` (
    ...
    `resetTokenExpiryTime` DATETIME(3) NULL,
...
CREATE TABLE `Product` (
    ...
    `status` ENUM('ARCHIVED', 'PUBLISHED') NOT NULL DEFAULT 'PUBLISHED',
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `Complaint` (
    ...
    `status` ENUM('OPEN', 'CANCELED', 'REVIEWED', 'RESOLVED') NOT NULL DEFAULT 'OPEN',
...
CREATE TABLE `CartItem` (
    ...
    `productSizeId` VARCHAR(191) NOT NULL,
...
CREATE TABLE `Order` (
    ...
    `status` ENUM('PENDING', 'PROCESSED', 'SENT', 'COMPLAINED', 'FINISHED', 'CANCELED', 'REJECTED') NOT NULL DEFAULT 'PENDING',
    `payment` ENUM('UNPAID', 'PAID', 'REJECTED') NOT NULL DEFAULT 'UNPAID',
    `paymentMethod` ENUM('TRANSFER', 'CREDITCARD', 'INDOMARET') NOT NULL DEFAULT 'TRANSFER',
    `paid_at` DATETIME(3) NULL,
    `payment_confirmed_at` DATETIME(3) NULL,
    `rejected_at` DATETIME(3) NULL,
    `no_resi` VARCHAR(191) NULL,
